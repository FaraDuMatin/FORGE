...
// The set of QUEUED projects in a pool that currently meet their bars. Feeds the
// allocation engine. Batched (no N+1) so it scales with the pool, not the row count.
export async function computeReadyIds(pool: Pool): Promise<Set<string>> {
